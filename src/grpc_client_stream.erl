...
-spec new(Connection :: pid(), Service :: atom(),
          Rpc :: atom(), Encoder :: module(),
          Options :: list()) -> {ok, Pid :: pid()} |
                                {error, Reason :: term()}.
...
call_rpc(Pid, Message, Timeout) ->
    try send_last(Pid, Message) of
        ok -> process_response(Pid, Timeout)
    catch
        _:_ ->
            {error,
             #{error_type => client,
...
handle_call({rcv, Timeout}, From,
    ...
    case {Value, StreamState} of
        ...
        {empty, _} ->
            {noreply,
             NewStream#{client => From, response_pending => true},
             Timeout}
    end.
...
new_stream(Connection, Service, Rpc, Encoder,
    ...
    RpcDef#{stream_id => StreamId, service => Service,
            rpc => Rpc, queue => queue:new(),
            response_pending => false, state => idle,
            encoder => Encoder, connection => Connection,
...
encode(#{encoder := Encoder, input := MsgType,
...
get_message(Headers, Pid, Timeout) ->
    case rcv(Pid, Timeout) of
        ...
        {error, timeout} ->
            {error, #{error_type => timeout, headers => Headers}}
    end.
...
get_trailer(Response, Headers, Pid, Timeout) ->
    case rcv(Pid, Timeout) of
        ...
        {error, timeout} ->
            {error,
             #{error_type => timeout, headers => Headers,
               result => Response}}
    end.
