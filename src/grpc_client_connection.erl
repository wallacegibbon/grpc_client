...
-export([new/4, stop/1, new_stream/1, new_stream/2, rst_stream/3,
         send_headers/3, send_body/4, ping/2, peercert/1]).
...
new_stream(#{http_connection := Pid, client := Client}, Options) ->
...
send_headers(#{http_connection := Pid, client := Client}, StreamId, Headers) ->
    Client:send_headers(Pid, StreamId, Headers, []).

send_body(#{http_connection := Pid, client := Client}, StreamId, Body, Opts) ->
    Client:send_data(Pid, StreamId, Body, Opts).

ping(#{http_connection := Pid, client := Client}, Timeout) ->
...
verify_server(Connection, Options) ->
    case
        proplists:get_value(verify_server_identity, Options, false)
    of
        true ->
...
validate_peercert(#{host := Host} = Connection, Certificate, Options) ->
    Server = proplists:get_value(server_host_override, Options, Host),
    case
        public_key:pkix_verify_hostname(Certificate, [{dns_id, Server}])
    of
        true ->
            {ok, Connection};
        false ->
