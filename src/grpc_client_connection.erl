...
-type connection() :: #{http_connection := pid(),
                        host := binary(), scheme := binary()}.

-type stream_id() :: integer().

-export_type([connection/0, stream_id/0]).

-type
     connection_option() :: grpc_client:connection_option().

-type stream_option() :: grpc_client:stream_option().

-spec new(Transport :: tcp | ssl, Host :: string(),
          Port :: integer(),
          Options :: [connection_option()]) -> {ok,
                                                connection()} |
                                               {error, term()}.

%% @doc Open a new http/2 connection and check authorisation.
new(Transport, Host, Port, Options) ->
    {GrpcOptions, H2Options} = process_options(Options,
                                               Transport),
    H2Client = proplists:get_value(http2_client,
                                   GrpcOptions,
                                   http2_client),
    ConnectResult = H2Client:new_connection(Transport,
                                            Host,
                                            Port,
                                            H2Options),
    case ConnectResult of
        {ok, Pid} ->
            verify_server(#{http_connection => Pid,
                            ...
                            scheme => scheme(Transport), client => H2Client},
                          GrpcOptions);
        _ -> ConnectResult
    end.

new_stream(Connection) -> new_stream(Connection, []).

-spec new_stream(Connection :: connection(),
                 Options :: [stream_option()]) -> {ok, stream_id()}.

new_stream(#{http_connection := Pid, client := Client},
           Options) ->
    Client:new_stream(Pid, Options).

rst_stream(#{http_connection := Pid, client := Client},
           StreamId, ErrorCode) ->
...
send_body(#{http_connection := Pid, client := Client},
          StreamId, Body, Opts) ->
    Client:send_data(Pid, StreamId, Body, Opts).

ping(#{http_connection := Pid, client := Client},
     Timeout) ->
...
stop(#{http_connection := Pid, client := Client}) ->
    Client:close(Pid).

-spec peercert(Connection :: connection()) -> {ok,
                                               Cert :: binary()} |
                                              {error, Reason :: term()}.
...
peercert(#{http_connection := Pid, client := Client}) ->
    Client:peercert(Pid).
...
verify_server(#{scheme := <<"http">>} = Connection,
              _) ->
    {ok, Connection};
verify_server(Connection, Options) ->
    case proplists:get_value(verify_server_identity,
                             Options,
                             false)
        of
        true -> verify_server_identity(Connection, Options);
        false -> {ok, Connection}
    end.
...
validate_peercert(#{host := Host} = Connection,
                  Certificate, Options) ->
                        ...
                        ssl ->
                            MandatorySslOpts =
                                ...
                                [{client_preferred_next_protocols,
                                  {client, [<<"h2">>]}}],
                            {_Ignore, ValidSslOpts} =
                                proplists:split(TransportOpts0,
                                                []),%%verify, fail_if_no_peer_cert
                            ValidSslOpts ++ MandatorySslOpts;
                        _ -> TransportOpts0
                    end,
    {H2Opts, GrpcOpts} = grpc_lib:keytake(http2_options,
                                          OtherOpts,
                                          []),
    {GrpcOpts,
     [{transport_options, TransportOpts} | H2Opts]}.
