...
-type stream_option() :: {metadata, metadata()} |
...
-type ok_response(Type) :: {ok,
                            #{result := Type, status_message := binary(),
                              http_status := 200, grpc_status := 0,
                              headers := metadata(), trailers := metadata()}}.

-type error_response(Type) :: {error,
                               #{error_type := error_type(),
                                 http_status => integer(),
                                 grpc_status => integer(),
                                 status_message => binary(),
                                 headers => metadata(), result => Type,
...
compile(FileName) -> grpc_client:compile(FileName, []).

-spec compile(FileName :: string(),
              Options :: gpb_compile:opts()) -> ok.
...
send(Stream, Msg) when is_pid(Stream), is_map(Msg) ->
...
stop_stream(Stream) -> stop_stream(Stream, 0).

-spec stop_stream(Stream :: client_stream(),
